...
#
# This version must be used if you enabled 2 factor authentication on your garmin account. Source it like this:
#
# source retrieve_garmin_tokens_2fas.sh <username> <password>
#
...
if [  -z "${_USERNAME}" ] || [ -z "${_PASSWORD}" ]; then
  ...
  curl 'https://sso.garmin.com/portal/api/mfa/verifyCode?clientId=GarminConnect&locale=en-US&service=https%3A%2F%2Fconnect.garmin.com%2Fmodern' -s \
    ...
    -H 'Content-Type: application/json' \
    -H 'Origin: https://sso.garmin.com' \
    -H 'Referer: https://sso.garmin.com/portal/sso/en-US/mfa?clientId=GarminConnect&service=https://connect.garmin.com/modern' \
    --cookie-jar .tmp/cookies.txt \
    --data-raw '{"mfaMethod":"'"$mfaMethod"'","mfaVerificationCode":"'"$token"'","rememberMyBrowser":false,"reconsentList":[],"mfaSetup":false}' | \
